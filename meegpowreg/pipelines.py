...
def make_pipelines(
    ...
    vec_riemann = make_column_transformer(
        *[(
            make_pipeline(
                ProjCommonSpace(scale=scale, n_compo=n_compo, reg=reg),
                Riemann(metric=metric)
            ), col)
            for col in fb_cols],
    ...
    vec_random = make_column_transformer(
        *[(
            make_pipeline(
                ProjRandomSpace(n_compo=n_compo),
                LogDiag()
            ), col)
          for col in fb_cols],
        remainder='passthrough'
    ...
    vec_spoc = make_column_transformer(
        *[(
            make_pipeline(
                ProjSPoCSpace(n_compo=n_compo, scale=scale, reg=reg,
                              shrink=shrink),
                LogDiag()
            ), col)
          for col in fb_cols],
